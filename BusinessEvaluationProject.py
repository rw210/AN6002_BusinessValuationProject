...
def investor_dcf(user_company_data, avg_growth_rate, time):
    ...
    if avg_growth_rate:
        revenue_growth_rate = avg_growth_rate
    else:
...
def user_company_valuation():
    try:
        ...
        avg_growth_rate = None  # Initialize avg_growth_rate
        ...
        dcf_value = investor_dcf(user_company_data, avg_growth_rate, time)
...
def V_Sent_model(X, y):
    # summary
    global V_model
    V_model = sm.OLS(y, X).fit()
    # xcol = X.iloc[:, 1:].columns.tolist()
    # print(V_model.summary(xname = ["Intercept"] + xcol))
    return V_model


model = V_Sent_model(X, y)


# predit the ci
def predict_growth(Ticker):
    # sentiment(2024)
    sent = company_sentiment_of_2024(Ticker)[2024]

    # financial data(2023)
    stock = yf.Ticker(Ticker)
    financials = stock.financials.T  # Transpose for easier access
    financials.index = financials.index.year  # Set year as index

    X = financials.loc[2023, ['Gross Profit', 'Operating Income', 'Net Income']]

    # case
    case = [1, sent].append(X)

    # Print model summary with confidence intervals
    # print(model.summary())
    ...
    growth = model.predict(case)
    CI_Lower = model.get_prediction(case).conf_int()[0][0]
    CI_Upper = model.get_prediction(case).conf_int()[0][1]
    # print('=' * 50)
    # print(f'Predicted growth: {growth}')
    # print(f'Confidence Interval: [{CI_Lower}, {CI_Upper}]')

    return growth, CI_Lower, CI_Upper
...
def combined_investor_analysis(ticker):
    ...
    ####
    # Set the date range for the macroeconomic data
    # 3. Fetch historical data for the S&P 500, DJIA, and NASDAQ
    sp500 = yf.Ticker("^GSPC").history(start="2019-01-01", end="2023-12-31")['Close'].resample('Y').mean()
    dji = yf.Ticker("^DJI").history(start="2019-01-01", end="2023-12-31")['Close'].resample('Y').mean()
    nasdaq = yf.Ticker("^IXIC").history(start="2019-01-01", end="2023-12-31")['Close'].resample('Y').mean()

    # Convert indexes to be timezone-naive
    ...
    fed_funds_rate = web.DataReader('FEDFUNDS', 'fred', start, end).resample('Y').mean()
    inflation_rate = web.DataReader('CPIAUCNS', 'fred', start, end).resample('Y').mean()
    gdp_growth_rate = web.DataReader('GDP', 'fred', start, end).resample('Y').mean()

    # Convert all indexes to be timezone-naive
    fed_funds_rate.index = fed_funds_rate.index.tz_localize(None)
    inflation_rate.index = inflation_rate.index.tz_localize(None)
    gdp_growth_rate.index = gdp_growth_rate.index.tz_localize(None)

    # 5. Merge the data
    # Prepare the dataframe for merging
    macro_data = pd.concat([sp500, dji, nasdaq, fed_funds_rate, inflation_rate, gdp_growth_rate], axis=1)
    macro_data.columns = ['S&P_500', 'DJI', 'NASDAQ', 'Fed_Funds_Rate', 'Inflation_Rate', 'GDP_Growth_Rate']
    macro_data.index = macro_data.index.year  # Set index to year for merging

    # Merge financial data with stock prices and macroeconomic indicators
    merged_data = pd.merge(all_data, yearly_price, left_index=True, right_index=True, how='inner')
    merged_data = pd.merge(merged_data, macro_data, left_index=True, right_index=True, how='inner')
    ...
    # 3. DCF Calculation
    growth, CI_Lower, CI_Upper = predict_growth(ticker)
    cashflow = stock.cashflow
    try:
